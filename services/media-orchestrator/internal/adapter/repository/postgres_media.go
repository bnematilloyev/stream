...
func (r *PostgresStreamMediaRepository) GetByStreamID(ctx context.Context, streamID uuid.UUID) (*domain.StreamMedia, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT stream_id, status, hls_path, playback_url, ingest_name, ffmpeg_pid, started_at, stopped_at
		FROM stream_media WHERE stream_id = $1
	`, streamID).Scan(&m.StreamID, &m.Status, &m.HLSPath, &m.PlaybackURL, &m.IngestName, &m.FFmpegPID, &m.StartedAt, &m.StoppedAt)
...
func (r *PostgresStreamMediaRepository) Upsert(ctx context.Context, m *domain.StreamMedia) error {
	_, err := r.pool.Exec(ctx, `
		INSERT INTO stream_media (stream_id, status, hls_path, playback_url, ingest_name, ffmpeg_pid, started_at, stopped_at, updated_at)
		...
		ON CONFLICT (stream_id) DO UPDATE SET
			...
			updated_at = NOW()
	`, m.StreamID, m.Status, m.HLSPath, m.PlaybackURL, m.IngestName, m.FFmpegPID, m.StartedAt, m.StoppedAt)
	return err
}
